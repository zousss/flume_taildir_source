...
public class SubLineDeserializer{

    private static final Logger logger = LoggerFactory.getLogger(SubLineDeserializer.class);
    ...
    SubLineDeserializer(Context context, ResettableInputStream in) {
        this.in = in;
    ...
    public Event readEvent(String schame) throws IOException {
        ensureOpen();
        ...
        if (line == null) {
            ...
            String[] recordvalue = line.split(",");
            for ( int i = 0;i<recordname.length;i++){

                linemap.put(recordname[i],recordvalue[i]);
            }

            return EventBuilder.withBody(linemap.toString(), outputCharset);
        }
    }
    ...
    public List<Event> readEvents(int numEvents,String schame) throws IOException {
        ensureOpen();
        List<Event> events = Lists.newLinkedList();
        for (int i = 0; i < numEvents; i++) {

            Event event = readEvent(schame);
            if (event != null) {
                events.add(event);
            } else {
                break;
